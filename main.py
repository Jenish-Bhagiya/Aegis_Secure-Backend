...
async def get_emails(account_email: str, user=Depends(get_current_user)):
    ...
    async with httpx.AsyncClient() as client:
        for msg in messages.get('messages', []):
            ...
            # === INTEGRATE REAL ML MODEL (FR-001) ===
            full_text = f"{subject}\n{body}"
            try:
                ml_resp = await client.post(
                    "https://cybersecure-backend-api.onrender.com/predict",
                    json={"text": full_text[:5000]}  # Truncate to avoid payload limits
                )
                if ml_resp.status_code == 200:
                    prediction = ml_resp.json().get("prediction", "ham")
                else:
                    prediction = "ham"  # Fallback on model error
            except Exception:
                prediction = "ham"  # Fallback on network error

            # Map to your threat format
            ...
            # === NFR-001 COMPLIANCE: DO NOT STORE RAW BODY ===
            result.append({
                "message_id": msg['id'],
                "subject": subject,
                "sender": sender,
                ...
                # Body is only used for detail view — not stored long-term
                "body": body
            })
